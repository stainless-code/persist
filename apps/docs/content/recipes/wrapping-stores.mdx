...
Wrap the root reducer with `persistableReducer` so hydrate can replace state (RTK slices ignore foreign actions otherwise). Alias if you also import redux-persist's `persistStore`.
