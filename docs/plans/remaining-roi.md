...
### 4. Real-browser + SSR + framework-runtime test matrix — Tier 3, M
...
### 5. React ergonomics layer — Tier 4, M-L
...
### 6. OPFS + SQLite-WASM + Cloudflare KV/Durable Objects adapters — Tier 4, M-L
...
### 7. StackBlitz / CodeSandbox playground — Tier 4, M
...
2. **#4 (real-browser + SSR matrix)** — M, de-risks the hydration-critical paths before more surface lands.
3. **#2 (examples/) → #3 (docs site) → #7 (playground)** — the docs/demo arc; sequence so each builds on the prior.
4. **#5 (React ergonomics) + #6 (OPFS/SQLite/Cloudflare)** — strategic; decide ship-vs-recipe per item.
