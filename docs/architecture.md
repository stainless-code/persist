...
- **`adapters/<seam>/`** — opt-in entries that own an optional peer and import only from `core/`:
  ...
  - `sources/` — `PersistableSource` adapters (tanstack-store, zustand, jotai, valtio, mobx, pinia). Shape-named, not library-named — same persistable shape → same name → same merge semantics; the subpath carries the library. Alias when importing two same-shape adapters into one module.
