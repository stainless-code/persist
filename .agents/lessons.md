...
- Docs `docs:audit` with `deployment.base` (e.g. `/persist`) still needs skips `canonical_bad_target` + `non_canonical_in_sitemap` + `indexable_page_not_in_sitemap` on Blume 1.1.2 (canonical vs stripped `page.url`); re-evaluate on bumps.
