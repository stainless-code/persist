...
- Release git auth needs a GitHub App token with Workflows write (`RELEASE_APP_CLIENT_ID` + `RELEASE_APP_PRIVATE_KEY` on the `release` environment) — `permissions.workflows` on `GITHUB_TOKEN` is invalid in workflow YAML (startup_failure / 0 jobs) and cannot authorize tag pushes when the tip range touched `.github/workflows/*`.
