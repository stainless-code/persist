...
- Release `changesets/action` tag push needs `permissions.workflows: write` — without it GitHub rejects the tag when the commit tree (or a concurrent main tip) touches `.github/workflows/*`, even after npm publish already succeeded.
