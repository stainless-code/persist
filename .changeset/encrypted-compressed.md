...
**Design note:** encryption + compression are backend **wrappers**, not sync `StorageCodec`s, because `crypto.subtle` and the stream APIs are async and the `StorageCodec` seam is sync. The codec serializes the envelope (sync); the wrapper encrypts/compresses the serialized string (async). 14 co-located tests (round-trip, ciphertext-not-plaintext, wrong-key-fails / compression-ratio, missing-key, formats, persistSource end-to-end, availability guard, dependency isolation).
