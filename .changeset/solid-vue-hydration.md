...
Add `./frameworks/solid` and `./frameworks/vue` hydration subpaths — `useHydrated(signal)` over the `HydrationSignal` seam, mirroring the React adapter (`./frameworks/react`).
